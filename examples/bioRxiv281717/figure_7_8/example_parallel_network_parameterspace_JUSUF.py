...
PSPACES['MPI5'].update(dict(
    # Population sizes
    POP_SIZE_REF=[12800, 2560],  # [12000, 2400],
...
PSPACES['POP'].update(dict(
    # Population sizes
    POP_SIZE_REF=[2560, 512], # [2400, 480],

    # allow different seeds for different network iterations
    GLOBALSEED=ps.ParameterRange([1234, 65135]), #, 216579876]),

    # MPI pool size
    MPISIZE=ps.ParameterRange([512]),
...
PSPACES['POP5'].update(dict(
    ...
    # MPI pool size
    MPISIZE=ps.ParameterRange([2560]),
...
# from here on we can run whatever command we want
unset DISPLAY # DISPLAY somehow problematic with Slurm
srun --mpi=pmi2 python -u example_parallel_network.py {}
'''
...
if __name__ == '__main__':
    ...
    for PSPACE in PSPACES.values():
        for pset in PSPACE.iter_inner():
            ...
            # create output directory for each ps_id
            if not os.path.isdir(os.path.join(OUTPUTDIR, ps_id)):
                os.mkdir(os.path.join(OUTPUTDIR, ps_id))
            ...
            # neuron count 2880)
            wt = 2560 * 512 / pset.MPISIZE * pset.POPSCALING * \
                np.sum(pset.POP_SIZE_REF) / 3072.
            wt = '%i:%.2i:%.2i' % (wt // 3600,
                                   (wt - wt // 3600 * 3600) // 60,
                                   (wt - wt // 60 * 60))
