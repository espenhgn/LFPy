...
class RecExtElectrode(object):
    ...
    periodic_order : uint
        for method == 'soma_as_point_periodic', domain order
    periodic_side_length : float
        for method == 'soma_as_point_periodic', domain side length
    ...
    def __init__(self, cell=None, sigma=0.3,
                 ...
                 seedvalue=None,
                 periodic_order=None, periodic_side_length=None,
                 **kwargs):
        ...
        self.periodic_order = periodic_order
        self.periodic_side_length = periodic_side_length
        ...
        elif method == 'soma_as_point_periodic':
            if self.anisotropic:
                self.lfp_method = lfpcalc.calc_lfp_soma_as_point_anisotropic
            else:
                self.lfp_method = lfpcalc.calc_lfp_soma_as_point
    ...
    def _loop_over_contacts(self, **kwargs):
        ...
        for i in range(self.x.size):
            if self.method != 'soma_as_point_periodic':
                ...
                E.append(self.lfp_method(self.cell, x=self.x[i], y=self.y[i], z=self.z[i],
                                               sigma=self.sigma,
                                               r_limit=self.r_limit))
                for j in range(-self.periodic_order, 0):
                    E.append(self.lfp_method(self.cell, x=self.x[i]+j*self.periodic_side_length, y=self.y[i], z=self.z[i],
                                                   sigma=self.sigma,
                                                r_limit=self.r_limit))
                for j in range(1, self.periodic_order+1):
                    E.append(self.lfp_method(self.cell, x=self.x[i]+j*self.periodic_side_length, y=self.y[i], z=self.z[i],
                                                   sigma=self.sigma,
                                                r_limit=self.r_limit))
                for j in range(-self.periodic_order, 0):
                    E.append(self.lfp_method(self.cell, x=self.x[i], y=self.y[i]+j*self.periodic_side_length, z=self.z[i],
                                                   sigma=self.sigma,
                                                r_limit=self.r_limit))
                for j in range(1, self.periodic_order+1):
                    E.append(self.lfp_method(self.cell, x=self.x[i], y=self.y[i]+j*self.periodic_side_length, z=self.z[i],
                                                   sigma=self.sigma,
                                                r_limit=self.r_limit))
                self.mapping[i, :] = np.array(E).sum(axis=0)
            ...
            for j in range(self.n):
                if self.method != 'soma_as_point_periodic':
                    ...
                    E.append(self.lfp_method(self.cell, x=x_n[j], y=y_n[j], z=z_n[j],
                                                   sigma=self.sigma,
                                                   r_limit=self.r_limit))
                    for i in range(-self.periodic_order, 0):
                        E.append(self.lfp_method(self.cell, x=x_n[j]+i*self.periodic_side_length, y=y_n[i], z=z_n[j],
                                                       sigma=self.sigma,
                                                    r_limit=self.r_limit))
                    for i in range(1, self.periodic_order+1):
                        E.append(self.lfp_method(self.cell, x=x_n[j]+i*self.periodic_side_length, y=y_n[i], z=z_n[j],
                                                       sigma=self.sigma,
                                                    r_limit=self.r_limit))
                    for i in range(-self.periodic_order, 0):
                        E.append(self.lfp_method(self.cell, x=x_n[j], y=y_n[j]+i*self.periodic_side_length, z=z_n[j],
                                                       sigma=self.sigma,
                                                    r_limit=self.r_limit))
                    for i in range(1, self.periodic_order+1):
                        E.append(self.lfp_method(self.cell, x=x_n[j], y=y_n[j]+i*self.periodic_side_length, z=z_n[j],
                                                       sigma=self.sigma,
                                                    r_limit=self.r_limit))
                    tmp = np.array(E).sum(axis=0)

                # store potential for different points on the contact
                if j == 0:
                    lfp_e = tmp
                else:
                    lfp_e += tmp
